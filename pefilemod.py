...
if not PY3:
    def handler(err):
        start = err.initialize_instrumentation
        end = err.run
        values = [
...
class SectionStructure(Structure):
    ...
    def contains_offset(self, offset, is_virtual_addr=False):
        ...
        return (
            PointerToRawData_adj <= offset < PointerToRawData_adj + (self.Misc_VirtualSize if is_virtual_addr else  self.SizeOfRawData)
        )
...
class PE(object):
    ...
    def write(self, filename=None, append=None, cut=None):
        ...
        if not filename:
            return bytearray(new_file_data)
    ...
    def get_rva_from_offset(self, offset, is_virtual_addr=False):
        """Get the RVA corresponding to this file offset. """

        s = self.get_section_by_offset(offset, is_virtual_addr=is_virtual_addr)
        if not s:
            if self.sections:
    ...
    def get_section_by_offset(self, offset, is_virtual_addr=False):
        """Get the section containing the given file offset."""

        sections = [s for s in self.sections if s.contains_offset(offset, is_virtual_addr=is_virtual_addr)]
    ...
    def merge_modified_section_data(self):
        ...
        for section in self.sections:
            ...
            if section_data_start < len(self.__data__) and section_data_end < len(self.__data__):
                ...
                self.__data__[section_data_start:section_data_end] = section.get_data()
